...
- **Added:** 2026-03-20 (seed) | **Last deep:** 2026-03-26T09:28 | **Pattern hits:** 9 | **SHA:** aa7daf0
...
- **Added:** 2026-03-20 (seed) | **Last deep:** 2026-03-26T09:28 | **Pattern hits:** 0 | **SHA:** 2e0f345
- **Notes:** SHA 077d9a8→2e0f345. Confirmed ticker (14th+ consecutive content-free change). 0 pattern hits across 14+ observations. Low-value for PATTERN_HUNT; retain for HORIZON_SCAN cross-reference only.

### bytedance/deer-flow
- **Why:** Multi-agent orchestration patterns from a major tech company
- **Look for:** Agent coordination, state management, tool orchestration, LLM provider patterns
- **Added:** 2026-03-21 (seed) | **Last deep:** 2026-03-26T09:28 | **Pattern hits:** 1 | **SHA:** d119214
...
- **Notes:** v10.6.2 active (Mar 21). SHA e2a2302→a656af2 changed. Their compress-filter-inject pipeline is more sophisticated than our simple state/ read/write. Could improve how we manage project_state.md context.
...
- **Notes:** v0.1.36+, very active. SHA 9a0fb95 unchanged. UI-focused. 1 pattern hit (SHA pinning). No new harness patterns.
...
- **Notes:** SHA c07ff8f→f547cec changed. Deep-dived 2026-03-26. ruflo-specific architecture, not directly adoptable. 0 harness patterns.
...
- **Notes:** SHA 8d96785 unchanged (3rd consecutive). 1 pattern hit (circuit breaker #76). Informational for #66.
...
- **Notes:** SHA b10b583 unchanged (3rd consecutive). Standard plugin format: .claude-plugin/plugin.json + commands/ + agents/ + skills/. Distribution channel for #66. 1 pattern hit (official plugin format).
...
- **Notes:** SHA 49840c2 unchanged (3rd consecutive). 1 pattern hit (multi-agent review quality gate). Self-learning pattern similar to our feedback-learner. evaluate-session.sh for session outcomes.
...
- **Notes:** SHA 55adfcb unchanged (3rd consecutive). Rust. v0.16.5 (Mar 23). Active development (675+ PRs). Could validate CLAUDE.md/SKILL.md in CI. 0 pattern hits.
...
- **Added:** 2026-03-24 (horizon scan) | **Observations:** 23 | **First seen:** 2026-03-24
- **Notes:** SHA 51494de unchanged (3rd consecutive). Python. v2.3.1. Validates #66 packaging (skills/ dir with symlinks). Architecturally most similar to tokenman. 0 direct pattern hits.
