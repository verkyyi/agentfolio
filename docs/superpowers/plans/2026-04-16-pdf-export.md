...
**Architecture:** A standalone GitHub Actions workflow (`pdf.yml`) triggers on changes to `data/adapted/*.json`, runs `resumed` with `jsonresume-theme-developer-mono` to produce PDFs alongside the JSON files. The React app checks for the PDF and renders a download button if available.

**Tech Stack:** `resumed`, `puppeteer`, `jsonresume-theme-developer-mono` (CI only), React + styled-components (download button)
  ...
  pdf:
    ...
    steps:
      ...
      - name: Install resumed and theme
        run: npm install -g resumed jsonresume-theme-developer-mono puppeteer
        ...
        run: |
          ...
          if [ "$SLUGS" = "all" ]; then
            for json in data/adapted/*.json; do
              ...
              resumed render "$json" --theme jsonresume-theme-developer-mono --type pdf --output "data/adapted/${slug}.pdf"
            done
          else
...
PDFs are generated by `.github/workflows/pdf.yml` when adapted JSON files change. Uses `resumed` with `jsonresume-theme-developer-mono`. PDFs live alongside JSON in `data/adapted/`.
```
