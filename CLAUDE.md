...
- **Resume theme:** `ResumeTheme.tsx` renders any valid JSON Resume document using styled-components. Based on jsonresume-theme-even. Supports all 12 JSON Resume sections.
...
PDFs are generated by `.github/workflows/pdf.yml` when adapted JSON files change. Uses `resumed` with `jsonresume-theme-even`. PDFs live alongside JSON in `data/adapted/`.
