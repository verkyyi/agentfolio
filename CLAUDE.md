...
├── pdf-theme/               # Vendored JSON Resume theme for PDF export (fork of jsonresume-theme-onepage with projects section)
...
- **PDF theme:** `jsonresume-theme-onepage-agentfolio` — vendored at `pdf-theme/` (fork of `jsonresume-theme-onepage` with a projects section) rendered via `resumed` CLI. Compact single-page layout. Installed in CI with `npm install -g --install-links ./pdf-theme` (file-install symlinks break handlebars resolution without `--install-links`).
